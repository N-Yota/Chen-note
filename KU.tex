...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
  この極限において、電子のLarmor回転は無視され、電子は単にy方向に$\bm{E}\times \bm{B}$ドリフトすることになる。これらの解を\eqref{4.119}式に代入することで、
...
\end{equation}
となる。この式の分母は、低周波かつ磁場に垂直な運動（$\perp$方向）に対するプラズマの誘電定数と等しい（付録\ref{Appendix1}参照）。
...
\begin{equation}
  \boxed{\frac{\omega^2}{k^2} = \frac{c^2}{\epsilon_r \mu_r}} \label{4.124.0}
\end{equation}
が得られる。

\eqref{4.124}式と\eqref{4.124.0}式を比較すると、$\mu_r=1$において、\eqref{4.124}式は単に誘電媒質中の電磁場の位相速度を与える。
プラズマの誘電定数を$\epsilon$とすると、

\[
  \frac{\omega}{k} = \frac{c}{\epsilon^{1/2}}
\]
実験室プラズマでは$\epsilon$は1よりずっと大きいので、近似的に、

\begin{equation}
  \frac{\omega}{k} = v_\phi = \frac{B_0}{(\mu_0\rho)^{1/2}} \label{4.125}
\end{equation}
と書ける。これらの波は、Alfven速度と呼ばれ、一定速度$v_A$で$\bm{B}_0$に沿って伝播する：
