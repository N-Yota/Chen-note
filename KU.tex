...
イオンの運動に関して、角$\pi/2 - \theta$はとても小さいため、$\bm{E}=E_1\hat{\bm{x}}$,$\nabla= ik\hat{\bm{x}}$ととってよい。一方、電子の場合、$\pi/2-\theta$が0であるのと、小さいが有限であるとでは大きな違いが生じる。
...
すると、$\bm{B}_0$に沿って波の中を動くことができ、その上下の移動で、x方向に動かずとも中性を保つように電荷を再配置できる。このように、$\pi/2-\theta$が有限であればDebye遮蔽を形成することができる。
...
「ほぼ」と表現したのは、これの臨界角があるためである。臨界角$\chi=\pi/2 - \theta$は、イオンと電子の平行速度の比に比例する：$\chi \simeq (m/M)^{1/2}$\,[rad]。これより大きな角$\chi$については以下の扱いが成り立ち、より小さな角$\chi$については\ref{section4.11}節で扱われる。
...
\]
縦波であれば、$\bm{B}_1 = 0$となり、波は静電波になったのである（\ref{section4.9}節）。
...
分散関係式\eqref{4.85}を図\refeq{fig4.26}に示す。この図はプラズマ波の図\refeq{fig4.5}に似ているが、図\refeq{fig4.26}の漸近速度$c$は図\refeq{fig4.5}の熱速度$v_{th}$よりもはるかに大きいため、分散関係は実際には大きく異なる。
...
$n$が大きすぎたり、$\omega$が小さすぎる場合、電磁波はプラズマを通過できない。このとき、式\eqref{4.85}から$k$が虚数となる：
