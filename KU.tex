...
\end{equation}
このドリフト$\bm{v}_R$は\textcolor{red}{湾曲ドリフト}と呼ばれる。
...
湾曲磁場では、湾曲ドリフトに加えてgrad B ドリフトも生じる。次にgrad B ドリフトを曲率半径方向の|B|の減少を考慮して計算してみよう。真空中ではマクスウェル方程式より、$\nabla \times \bm{B} = 0$が成り立っている。
